...
def line_sort(lines, last_midpoint_coord, lim):
    ...
    # print("huh", type(sorted_idxs), sorted_idxs, sorted_idxs.reverse())
...
# def update_coords(p, dict, point_list):
#     if p.fwd not in dict and p.fwd != -1:
#         print(dict[p.id])
#         dict[p.fwd] = [dict[p.id][0] + 1., dict[p.id][1]]
#         update_coords(get_point_by_id(p.fwd, point_list), dict, point_list)
#     if p.back not in dict and p.back != -1:
#         dict[p.back] = [dict[p.id][0] - 1., dict[p.id][1]]
#         update_coords(get_point_by_id(p.back, point_list), dict, point_list)
#     if p.left not in dict and p.left != -1:
#         dict[p.left] = [dict[p.id][0], dict[p.id][1] + 1.]
#         update_coords(get_point_by_id(p.left, point_list), dict, point_list)
#     if p.right not in dict and p.right != -1:
#         dict[p.right] = [dict[p.id][0], dict[p.id][1] - 1.]
#         update_coords(get_point_by_id(p.right, point_list), dict, point_list)
...
def h_line_similar(prev, hv_arr):
    ...
    # print("diff1", angle_diff(prev, ang1), "diff2", angle_diff(prev, ang2))
    # print("ang1, ang2: ", ang1, ang2)
    if abs(angle_diff(prev, ang1)) < abs(angle_diff(prev, ang2)):
        return True
    else:
        # print("change occcccurreddd")
        return False
